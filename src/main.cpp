...
int main() {
  ...
  double sensor_range = 50;  // Sensor range in [m]
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (!pf.initialized()) {
            ...
            // Predict the vehicle's next state from 
            // previous (noiseless control) data
          ...
          // Sense observations in JSON format 
          ...
          // Calculate and output the average weighted error of 
          // the particle filter over all time steps so far
